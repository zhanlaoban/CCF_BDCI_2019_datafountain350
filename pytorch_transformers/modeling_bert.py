...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        #self.W = nn.Linear(config.lstm_hidden_size*2, config.lstm_hidden_size*2)
        #self.gru = nn.GRU(config.hidden_size, config.lstm_hidden_size, num_layers=1,bidirectional=True,batch_first=True).cuda()
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        #print(input_ids.shape)  #torch.Size([4, 3, 256])    [batch_size, split_num, max_seq_length]
        flat_input_ids = input_ids.view(-1, input_ids.size(-1))
        #print(flat_input_ids.shape) #torch.Size([12, 256])     [batch_size*split_num, max_seq_length]
        ...
        pooled_output = outputs[1]
        #pooled_output.shape: torch.Size([12, 768]), [batch_size, hidden_size]
        

        
        output = pooled_output.reshape(input_ids.size(0),input_ids.size(1),-1).contiguous()
        
        #output.shape  [4, 3, 768]  [batch_size, split_num, hidden_size]
...
class BertForSequenceClassificationFC(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels - 1]``.
    ...
    """
    def __init__(self, config):
        super(BertForSequenceClassificationFC, self).__init__(config)
        self.num_labels = config.num_labels
